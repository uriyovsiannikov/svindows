...
void KeSetKernelStack(UINT64 kernel_rsp)
{
    ...
    /* Re-assert the swapgs target on every switch. A thread that terminates
     * mid-syscall never runs the exit swapgs, which would otherwise leave
     * KERNEL_GS_BASE wrong for the next user thread; refreshing it here keeps
     * syscall entry correct regardless. (Single-CPU; one KPCR.) */
    wrmsr(MSR_KERNEL_GS_BASE, (UINT64)&g_kpcr);
}

void KiInitializeSystemCalls(void)
{
    /* GS: kernel per-CPU block becomes active after the first swapgs on entry. */
    wrmsr(MSR_GS_BASE, 0);
    wrmsr(MSR_KERNEL_GS_BASE, (UINT64)&g_kpcr);
